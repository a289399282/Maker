...
@Service("StudiomemServiceImpl")
public class StudiomemServiceImpl implements StudiomemService {
	@Autowired
...
//				return ResultUtil.error("你已提交申请,请不要重复申请");
	...
	public Result updateStudiomem(Studiomem studiomem) {
		...
		studiomem2.setStumemSum(studiomem.getStumemSum());
		//List<Studiomem> studiomems = studiomemDao.selectStudiomem(studiomem2);

		studiomem2 = studiomemDao.selectOneStudiomem(studiomem2);
		if (studiomem2!=null) {
			if (studiomem2.getStumemPower()==3) {
				return ResultUtil.error("此用户无法被修改");
			}
			//判断地否是修改成同样的状态
			...
			//判断地否是修改成同样的权限
			if (studiomem2.getStumemPower().equals(studiomem.getStumemPower())) {
				return ResultUtil.error("已是这种权限");
			}
			//如果是同意,把power码改成1
			if (studiomem!=null && studiomem.getStumemIs() == 2) {
		...
		if (size!=0) {
			...
			try {
				...
				for (int i=0;i<size;i++) {
					...
					studiomemCut.setDevName(developer.getDevName());
					studiomemCut.setStuName(studio.getStuName());
					studiomemCut.setStuSpeciality(studio.getStuSpeciality());
					stuCutlist.add(studiomemCut);
				}
			} catch (IllegalAccessException e) {
				e.printStackTrace();
