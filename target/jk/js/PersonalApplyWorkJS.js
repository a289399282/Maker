    $(function () {
    	$('#ApplyWorkTable').bootstrapTable('destroy'); 
            //初始化Table
        ...
        var TableInit = function () {
            ...
            oTableInit.Init = function () {
                $('#ApplyWorkTable').bootstrapTable({
                    url: '/Maker/positionmem/selectPosmemMe',//请求后台的URL（*）
                    method: 'post',//请求方式（*）
                    ...
                    columns: [
                       	{
                            ...
                            field: 'positionmemState',
                            title: '申请状态',
	                        formatter: function (value, row, index) {
	                          var a="";
	                          if(row.positionmemState == "0"){
	                          	a="已撤销申请";
	                          }else if(row.positionmemState == "1"){
	                          	a="申请中"
	                          }else if(row.positionmemState == "2"){
	                          	a="已录取"
	                          }else if(row.positionmemState == "3"){
	                          	a="已辞职"
	                          }else{
	                          	a="错误"
	                          }
                            ...
                            title: '操作',
                            formatter: function (value, row, index) {
                             var d='<a onclick="removeID(\''+row.positionId+'\',\''+row.positionmenSum+'\')" title="撤销申请" style="cursor: pointer">撤销申请</a> ';
                                return d;
                            }
                        },
                    ],
...
//                  offset: params.offset,
    ...
    function removeID(id,sum) {
            	var r = confirm('确定要撤销申请？');
            	if(r==true){
            	$.ajax({
                    type: "post",
                    url: "/positionmem/updatePosmemStart",
                    data: {
                        positionId: id,
                        positionmemState  : 0,
                        positionmenSum : sum
...
//      }
        ...
        /*刷新table*/
        function reLoad() {
            $('#ApplyWorkTable').bootstrapTable('refresh');
        }
...
function CheckDetails(Id){
	   $('#CheckDetailsModel').modal({backdrop: "static"})
	    $.ajax({
        type: "post",
        url: "/Maker/position/selectPosById",   //查看详情URL
        data: {
            positionId : Id
        },
